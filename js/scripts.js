(function($) {
    ...
    var portfolioRefresh = function() {
        ...
        setTimeout(function() {
            $port.isotope({masonry: {columnWidth: width}});
            $port.isotope('layout');
        }, 200);
    };
    $('.portfolio').isotope({
    ...
    $('[data-parallax]').fitBackground('data-parallax', {forceFixed: true});
    ...
    if(0 < $('#hero').length) {
        ...
        switch(true) {
            case $hero.get(0).hasAttribute('data-solid'):
                $hero.css({backgroundColor: $hero.attr('data-solid')});
                break;
            case $hero.get(0).hasAttribute('data-pattern'):
                $hero.css({backgroundImage: 'url("' + $hero.attr('data-pattern') + '")'});
                break;
            case $hero.get(0).hasAttribute('data-fixed-pattern'):
                $hero.css({backgroundImage: 'url("' + $hero.attr('data-fixed-pattern') + '")', backgroundAttachment: 'fixed'});
                break;
            case $hero.get(0).hasAttribute('data-image'):
                $hero.fitBackground('data-image', {viewport: $hero, forceScroll: true});
                break;
            case $hero.get(0).hasAttribute('data-fixed-image'):
                $hero.fitBackground('data-fixed-image', {viewport: $hero, forceFixed: true});
                break;
            case $hero.get(0).hasAttribute('data-video'):
                ...
                $hero.find('.fullscreen-object > *').fitObject('data-video', {viewport: $hero, ratio: parseFloat(eval($hero.attr('data-video-ratio')))});
                break;
            case $hero.get(0).hasAttribute('data-youtube'):
                ...
                $hero.find('.fullscreen-object > *').fitObject('data-youtube', {viewport: $hero, ratio: 640/390});
                break;
            case $hero.get(0).hasAttribute('data-youtube'):
                $hero.prepend('<div class="fullscreen-object"><iframe src="https://www.youtube.com/embed/' + $hero.attr('data-youtube') + '?autoplay=1&modestbranding=1&controls=0&showinfo=0&rel=0&enablejsapi=1&version=3&allowfullscreen=true&wmode=transparent&iv_load_policy=3&html5=1" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>');
                $hero.find('.fullscreen-object > *').fitObject('data-youtube', {viewport: $hero, ratio: 640/390});
            ...
            case $hero.get(0).hasAttribute('data-vimeo'):
                ...
                $hero.find('.fullscreen-object > *').fitObject('data-vimeo', {viewport: $hero, ratio: 960/408});
                break;
        }
    }
})(jQuery);